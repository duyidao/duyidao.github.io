...
2. [Yarn (opens new window)](https://yarnpkg.com/)会缓存它下载的每个包所以无需重复下载
   ...
   ```
3. 使用 [Create React App (opens new window)](https://create-react-app.dev/)安装<word text="React"/>项目非常方便，下面来创建项目 `daodao`
   ```sh
...
下面在 `index.js` 入口文件中创建组件最简单的组件。下面的组件像<word text="HTML" />但只是像而已。它被称为 JSX 是一个<word text="JavaScript" />的语法扩展，它具有<word text="JavaScript" />的全部功能。
...
我们知道<word text="JavaScript" />中的类也是函数，<word text="React" />也可以使用类的方式声明组件，但要保证返回 JSX 组件标签
...
根据件就像<word text="HTML" />标签中的 **html** 一样，所有其它标签都在它的里面。根组件也是这个特点，在里面构建不同组件产生不同界面。

组件一般都是独立的文件，下面创建 `App.js` 文件构建根组件，在入口文件中导入组件并渲染。
...
组件中的注释使用<word text="JavaScript"/>注释规范，因为是<word text="JavaScript"/>所以要使用花扩号包裹。
...
<word text="React"/>中定义样式也非常简单，下面是定义 `STYLE` 行样式
...
当然也可以使用<word text="JavaScript"/>程序计算，下面是使用三元表达式的计算
