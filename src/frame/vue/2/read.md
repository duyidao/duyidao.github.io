...
该方法的意义不大，逻辑可以写到 `create` 钩子函数内，获取<word text="DOM" />元素并操作的方法可以写到 `mounted` 钩子函数内。
...
接受两个参数，参数一是<word text="Vue" />实例，参数二是传的数据参数，如下图所示：
...
2. 调用内部的 `toArray` 方法把传递的参数从索引为 1 开始提取出来作为一个数组（即不要索引为 0 的<word text="Vue" />参数）
3. 在该数组中第一项添加 `this`
...
对于第一个参数要传<word text="Vue" />实例或许会有疑问，它是为了处理版本问题而存在的。以刚刚的案例为例，假设 `plugin` 的<word text="Vue" />版本是 1.4， 而 Vue.use 的版本是 2.4，此时版本不一致。

因此传<word text="Vue" />的目的就是将<word text="Vue" />的构造函数传递给插件中，让所有插件依赖的<word text="Vue" />是同一个版本。

总结一下：

- `use` 方法目的是将<word text="Vue" />的构造函数传递给插件中，让所有插件依赖的<word text="Vue" />是同一个版本
- 默认调用插件或插件的 `install` 方法
- `vuex` 和 `vue-router` 里 `package.json` 的依赖里面没有<word text="Vue" />，是通过参数传进去的
