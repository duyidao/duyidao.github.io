...
最终可以看到所有内部样式和外部样式规则，每一个规则包含选择器和样式 `style` 。如果想要手动修改样式，可以通过 `addRule` 方法修改。具体代码如下：
...
合成线程会对每个图层进行分块，并把分块信息交给光栅化线程池。光栅化线程池会把块信息转换成位图。合成线程拿到每个图层的位图后，会生成一个个「指引(quad)」信息，优先光栅化靠近视口区域。最后交给 GPU 画。
