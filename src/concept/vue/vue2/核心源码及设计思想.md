...
测试一下是否全局有了<word text="Vue"/>实例，`dist` 文件夹下新建一个 `index.html` 文件，引入 `vue.js` 文件，在输出<word text="Vue"/>，代码如下：
...
>
> 因为<word text="Vue2"/>的 `Object.defineProperty` 语法只支持 IE9 以上，没有低级语法可以代替；而<word text="Vue3" />的 Proxy 更是直接剔除 IE 行列。

## Vue响应式原理实现

实现响应式数值变化，数值变化了我们可以监控到数据的变化。

首先初始化数据，在 `index.html` 内创建一个<word text="Vue"/>实例，把所有数据放到 `data` 对象内，代码如下：
...
接下来需要创建一个。<word text="Vue" />源码并没有创建类 `class` 来设计，因为当方法多了之后，会造成很多方法的耦合。<word text="Vue" />采取的方法是配置构造函数，通过原型的方式把构造函数挂载到<word text="Vue"/>实例上，最终导出<word text="Vue"/>实例。如下：
...
在 `index.html` 文件中 `new Vue` 就是此处导出的<word text="Vue"/>。
...
如果是对象，不需要作额外处理；如果是函数，则通过 `call()` 方法修改其 `this` 指向为<word text="Vue"/>上。代码如下所示：
...
定义一个类方法 `Observe` ，构造器中获取传参，并在 `walk` 方法中 **重新定义** 对象的属性。这也是为什么<word text="Vue2"/>性能比<word text="Vue3" />差的原因。
...
1. 模板引擎，性能差，需要正则匹配替换，Vue1.0 版本没有引入虚拟<word text="DOM"/>
2. 虚拟<word text="DOM"/>，数据变化后比较虚拟<word text="DOM"/>的差异，最后更新需要更新的地方
3. 核心是需要将模板变成<word text="JavaScript"/>语法，通过<word text="JavaScript"/>语法生成虚拟<word text="DOM"/>
...
2. 没有 `render()` 函数则需要判断是否有 `template` 模板标签，有的话通过函数转化为 `ast` 树
...
在<word text="HTML" />主要解析标签、文本、属性、表达式，首先在 `src` 文件夹下新建一个 `compiler/index.js` 文件，用于解析语法转为 ast 树。

接着创建正则，通过正则匹配开始标签、属性、闭合标签和表达式或文本内容，代码如下：

```js [compiler/index.js]
...
紧接着创建 `parseHTML` 函数，用于通过正则 `.match()` 方法解析<word text="HTML" />，文本解析规则如下：
...
如果还是无法理解接下来用图解的方式来说明，先看一段<word text="HTML" />代码：
...
此时开始循环匹配，匹配第一个是 `<div id="app"` 开始标签，往栈中追加该 `div` ，创建 AST 树，并把它设为根节点 `root` 。然后截取掉该 `div` ，<word text="HTML" />代码变为了：
...
截取去除相应<word text="HTML" />代码后接着循环，匹配到结束标签，剔除 `span` ，最后匹配到 `div#app` 的结束标签，把该节点也从栈中剔除，最后 AST 树创建完毕。

## 代码生成虚拟DOM

接下来转为虚拟<word text="DOM" />元素，虚拟<word text="DOM" />包含三个方法：
...
处理 `gen` 函数中为文本的判断逻辑，首先创建一个数组 `tokens` ，创建好一个虚拟<word text="DOM" />后追加到数组内，后续数组转字符串拼接的形式输出出去。
...
返回 `compileToFunction` 函数，此时已经能获取到虚拟<word text="DOM" />了，返回一个 `with` 函数。代码如下：
...
此时原型上是没有这些 `_c` 、`_v` 的方法的，因此运行刷新后会报错。现在给<word text="Vue" />原型绑定这些方法。

前面已经介绍过每个方法的功能与作用，这里不做过多赘述，新建一个 `lifecycle.js` 文件，主要做以下流程：

1. 创造响应式数据
2. 模板转换成 `ast` 语法树
3. 将ast语法树转换了 `render` 函数
4. 后续每次数据更新可以只执行 `render` 函数（无需再执行 `ast` 转化过程

`render` 会产生虚拟节点（响应式数据）。根据生成的虚拟节点创造真实<word text="DOM" />
...
新建 `ndom/index.js` 文件，用于创建虚拟<word text="DOM" />节点，代码如下：
...
> [!NOTE] 题外话
>
> AST 树与 vnode 虚拟<word text="DOM" />节点是不同的含义，AST 树做的是语法层面的转换，描述的是语法本身（可以描述js、css、html）。而 vnode  虚拟<word text="DOM" />描述的是<word text="DOM" />元素，可以增加一些自定义属性

### 实现转换

接下来修改原型上的 `_update()` 方法实现转换。首先获取到旧<word text="DOM" />节点和新的虚拟<word text="DOM" />，代码如下：
...
`patch` 方法主要用于判断是真实<word text="DOM" />还是虚拟<word text="DOM" />，真实<word text="DOM" />则创建新的<word text="DOM" />，替换掉旧的<word text="DOM" />。
...
3. 调用 `render` 方法函数，进行取值操作，产生对应的虚拟<word text="DOM" /> `render() {_c('div', null, _v(name))}` ，触发 `get` 方法
4. 将虚拟<word text="DOM" />渲染成真实<word text="DOM" />
...
由于是异步更新，因此修改完值后<word text="DOM" />元素的数据有时并未能及时获取到最新值，而直接用定时器 `setTimeout` 又不够优雅，Vue2 提出的 `$nextTick` 方法则采用了优雅降级的方法：

1. `nextTick` 中不是直接使用定时器API，而是采用优雅降级的方法，放到队列中是同步，单独开是异步
...
因此写法可以给<word text="Vue" /> 函数的原型上添加 `$watch` 的方法，所有写法最终都调用该方法。
...
1. 双方都有子节点（数组长度大于0）则完整 diff 算法比较，比较二者的儿子
...
如果新节点的开始索引到最后小于等于新节点的结束索引说明新节点有多的虚拟<word text="DOM" />，多余的追加进去。

如果旧节点的开始索引到最后小于等于旧节点的结束索引说明旧节点有多的虚拟<word text="DOM" />，多余的删除。
...
在<word text="Vue2"/>中，提供了一个 [extend](https://v2.cn.vuejs.org/v2/api/) 方法手动挂载组件，官方文档Demo使用示例代码如下：
...
因此需要为<word text="Vue"/>全局挂载一个 `extend` 方法，该方法是一个函数。示例代码中 `new` 调用了组件实例后调用 `$mount()` 方法挂载，而这个方法在<word text="Vue"/>原型上，因此可以把<word text="Vue"/>的原型挂载到 `extend` 下的子类原型上。
...
本质上就是创造了一个子类，继承了<word text="Vue"/>内的原型方法。每一次创建实例，都会 `new Sub` ，初始化子组件，把子组件的选项赋值给 `Sub.options` 上。如果 `data` 是一个对象形式，则会相互影响。

> [!WARNING] 注意
>
> 第 10 行代码必须要有，因为 `Object.create` 方法会造成一个潜在的 BUG ，继承后方法 `Sub` 原型上的构造器就不是自己的了，因此要指向自己。

在<word text="Vue"/>中有一个全局 `component` 方法，用于创建组件，因此收集对应的 `id` 和 `definition` ，代码如下：
