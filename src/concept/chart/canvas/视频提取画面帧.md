...
视频提取画面帧主要操作是拿到视频的画面后，把画面画到<word text="Canvas" />上，<word text="Canvas" />可以拿到文件对象 <word text="Blob"/> 和 `url` 。而<word text="Canvas" />不能直接画一个文件，需要使用 `video` 这类多媒体元素画到<word text="Canvas" />内。
...
1. 创建一个 `video` 标签
2. 通过 <word text="URL.createObjectURL"/> 获取 object url 。以 `blob:` 为前缀，复制粘贴到新的网页打开就能看到视频
...
5. 通过 `toBlob` 方法转为 <word text="Blob"/> ，`return` 返回 <word text="Blob"/> 和 `url`
...
之所以使用 <word text="URL.createObjectURL"/> 而不是转为 <word text="base64"/> ，是因为如果视频文件过大，要全部转为 <word text="base64"/> 的话，会很耗时。
