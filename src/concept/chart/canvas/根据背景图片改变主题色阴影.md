...
1. 任何颜色都可以由 `r`、`g`、`b`、`a` 四个通道表示。`r` 为红色系最大值为 255；`g` 为绿色系最大值为 255；`b` 为蓝色系最大值为 255；`a` 为透明度，最大值也为 255
2. 使用 <word text="Canvas"/> 读取图片节点
3. 利用 <word text="Canvas"/> 的 `getImageData` 获取图片每一个像素点的 `rgba` 值，以一个大数组的格式返回
4. 判断图片的主题色可以通过 `rgb` 值算出来平均色
...
onMounted(() => {
  ...
  // 图片dom挂载，但是图片需要加载，要在图片加载好了才能画到canvas
  imgDom.onload = () => {
