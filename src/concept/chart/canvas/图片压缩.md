...
`toBlob()` 是 `<canvas>` 元素上的一个方法，用于将当前画布内容转换为一个 <word text="Blob"/> 对象，<word text="Blob"/> 对象可以用于各种用途，例如上传到服务器或保存为本地文件。
...
| `type`（可选）    | 指定生成的 <word text="Blob"/> 对象的 <word text="MIME"/> 类型，默认为 `image/png`。                             |
...
> [!WARNING] 注意
> `toBlob()` 方法是 <word text="HTML5"/> 新增的方法，不是所有的浏览器都支持它。在使用之前，最好检查一下浏览器的兼容性。

## 获取图片

首先先获取用户选择上传的图片，前端文件上传与相关操作方法复习可点击 [前端文件上传与相关操作](/handle/business/前端文件上传与相关操作) 。代码如下所示：
...
保存运行后，选择图片可以成功渲染到页面上。现在利用 <word text="Canvas"/> 转 <word text="Blob"/> 或 <word text="DataURL"/> 格式时实现图片压缩。步骤如下：

1. 创建一个 <word text="Canvas"/> 真实 <word text="DOM"/>，并获取到图片 <word text="DOM"/> 的宽高
2. 通过 <word text="Canvas"/> 的 `getContext('2d')` 方法法创建获取 2D 渲染上下文
3. 通过 `drawImage()` 在 `<canvas>` 元素上绘制图像
4. 使用 `toBlob()` 方法把 <word text="Canvas"/> <word text="DOM"/> 节点转换为一个 <word text="Blob"/> 对象
5. 把转换好的 <word text="Blob"/> 对象通过第三方库 <word text="file-saver"/> 的 `saveAs` 方法转为图片格式
6. 做其他业务处理（如发请求）
