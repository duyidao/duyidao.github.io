...
Echart 饼图中间内容允许自定义，效果：

![示例](https://pic.imgdb.cn/item/65621720c458853aef11e1d6.jpg)

### Echart 实现

实现该功能，需要在饼图的 `series` 数组对象的 `label` 属性中设置一个 `normal` 属性，其中：
...
查看 ECharts 官方文档，发现他有一个方法 `legendselectchanged`，文档[Documentation - Apache EChartss](https://echarts.apache.org/zh/api.html#events.legendselectchanged) 。该方法在点击切换图例的选中状态时会触发。

### Echart 事件绑定

为渲染图例的 DOM 元素绑定事件，代码
...
拿到数据后就可以在自定义饼图中间内容的自定义函数内修改内容。代码：
...
逻辑：渲染第一项选中的数据，如有高、较高、中、较低、低五条数据，点击中和低后，第一项选中的数据是高，则中间内容继续渲染高。
