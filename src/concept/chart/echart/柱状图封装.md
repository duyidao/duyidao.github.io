...
Y 轴单位由 `yAxis` 属性的 `name` 字段控制，因此代码修改为：
...
其中，柱体名称、类型、柱体宽度可以写死，后三者动态传入，逻辑处理为获取到数据后，先渲染长度为传参的数据长度、每一项数据为0的图表，然后依次获取到对应的索引的数据。
...
从易用性和自主性方面考虑，可以添加短路运算，如果父组件有传相应的数组，则使用父组件传过来的数组；否则采取 “过滤出所有数据的桩号并保存为新数组” 的方式。
        ...
        series: props.info.seriesData.map((item, index) => {
          ...
          return {
            ...
            emphasis: {
              ...
              itemStyle: {
                color:
                  item.emphasisColor || item.itemColor || 'rgb(68, 218, 30)',
              },
            },
          }
        }),
