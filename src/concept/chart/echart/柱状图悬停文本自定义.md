...
修改纵轴的参数，只需要修改 `yAxis` 对象的值即可生效。实现该效果，只需把 `minInterval` 属性设为1，最低单位为1，就不会出现小数的情况。

### Echart 纵轴线不显示，横轴线为虚线

实现该效果，需要针对 `xAxis` 和 `yAxis` 两个数据对象进行设置，代码：
...
自定义文本需要通过 `formatter` 函数方法实现，在 X 轴上实现则在 `xAxis` 上添加即可。代码：
...
### Echart 鼠标悬停显示同组柱体的数据

与鼠标悬停有关的效果都在 `tooltip` 属性对象内设置，通过 `formatter` 函数返回需要显示的内容。可以返回一个包含 `HTML` 标签的字符串。

函数的形参 `params` 可以接收 `series` 与 `dataset` 内的柱体数据，包括数组数据、柱体颜色等，因此可拿到数据渲染文本内容，并且可以设置样式。
