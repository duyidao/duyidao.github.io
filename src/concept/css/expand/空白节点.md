...
在 CSS 中有两个重要属性，`line-height` 和 `vertical-align` 都与基线有关，`line-height` 行高的定义就是两个基线之间的距离，`vertical-align` 的默认值就 `baseline`，也就是基线对齐。对于内联元素，需认识到，虽然 `vertical-align` 与 `line-height` 看不见，但实际上**到处都是**！
