...
1. `document.startViewTransition()` 方法开始过渡动画，接收一个函数作为参数，该函数用于修改 DOM 结构（即修改 `className` 部分），得到一个参数，命名为 `transition`
...
1. 修改中心点为鼠标点击的位置
2. `clip-path` 的 `circle` 半径要计算修改，否则中心点在四个角时，会因为半径不够，导致后续没有动画
