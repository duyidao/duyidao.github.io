...
1. 相框显示，鼠标移入后显示全部，移出则裁剪为四方形。使用多边形裁剪 `polygon` ，代码：
   ...
   本质是把四个点的横纵坐标移动一下。推荐多边形 `clip-path` 的在线样式裁剪网址：[CSS3 剪贴路径（Clip-path）在线生成器工具](http://tools.jb51.net/code/css3path) 。

2. 眨眼效果，该效果主要通过动画搭配椭圆裁剪 `ellipse` 实现。设置动画，在不同的关键帧定义不同的椭圆半径，代码：
