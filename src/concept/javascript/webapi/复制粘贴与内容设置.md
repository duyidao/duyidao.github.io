...
- 对象，指 `navigator.clipboard` ，可用于写入和读取剪贴板
- 事件，有两个事件：
...
通过 `navigator.clipboard.readText()` 方法读取剪贴板的内容，并将其赋值给 `document.body.innerHTML`。该方法返回的是一个 `Promise` 对象，所以需要使用 `.then()` 方法来获取读取到的内容。
