...
项目变大时需要把不同的业务分割成多个文件，这就是 <word text="模块化" /> 的思想。模块是比对象与函数更大的单元，使用模块组织程序便于维护与扩展。

生产环境中一般使用打包工具如 `webpack` 构建，他提供更多的功能。但学习完本章节后学习打包工具会变得简单。
...
在浏览器中使用以下语法将之脚本做为模块使用，这样就可以在里面使用模块的代码了。
...
```js
<script type="module">fn = "daodao"; // Error</script>
```
...
文件内容如下 # 1.1.js let fn = "daodao"; # 1.2.js console.log(fn) // 报错
```
...
#fn.js内容如下 console.log("daodao.com");
```
...
```js
<script type="module">
  import * as api from "./fn.js"; console.log(api.site); console.log(api.User);
</script>
```
