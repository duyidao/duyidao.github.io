...
`Promise` 可以理解为承诺，就像我们去 KFC 点餐服务员给我们一张取餐票，这就是承诺。如果餐做好了叫我们这就是成功，如果没有办法给我们做出食物这就是拒绝。
...
async function user() {
  ...
  let lessons = await ajax(`http://localhost:8888/php/daodao.php?id=${user.id}`)
  console.log(lessons)
}
```
