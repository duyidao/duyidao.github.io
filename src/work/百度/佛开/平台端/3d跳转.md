---
title: 百度外包佛开项目3d跳转
titleTemplate: 百度外包佛开项目3d跳转
description: 地图 百度 佛开 3d
head:
  - - meta
    - name: description
      content: 百度外包佛开项目3d跳转
  - - meta
    - name: keywords
      content: 地图 百度 佛开 3d
pageClass: baidu-fokai-3d
tags: preserve-3d
---
...
根据效果图不难推测出，整体的旋转切换效果需要通过 _3D 动画_ 来实现，通过 `rotate` 来控制。现在来思考如何控制它们的背景及不透明度和旋转的角度。
...
<template>
  <div
    ...
    :class="{ active: isActive(item) }"
    :style="{
      '--opacity': 1 - Math.abs(getGap(activeIndex, index)) * unitOpacity * 1.7,
    }"
  >
    <!-- ... -->
  </div>
...
const activeIndex = computed(() =>
  list.findIndex((item) => item.url === router.path)
)
...
剩下 3D 旋转 的功能需要实现，先实现 3d 布局，就是为需要 3d 旋转的盒子设置 `transform-style: preserve-3d;` 样式，通过 `rotate` 实现角度调整。
...
const activeIndex = computed(
  (e) => props.systemList?.findIndex?.((el) => el.url && isActive(el)) ?? 0
)
