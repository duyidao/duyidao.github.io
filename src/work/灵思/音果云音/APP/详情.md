...
音频详情页需要一个播放音频的控件，在最开始的时候使用的是 <word text="UniApp" /> 提供的 `audio` 组件，在 `app` 上运行无误，但是在 `h5` 上运行时报错，发现其组件停止维护。根据项目经理的要求，为了不给后续埋雷，改成使用 <word text="UniApp" /> 内置 API `uni.createInnerAudioContext()` 。
...
- `paused` ：当前是否暂停或停止状态，`true` 表示暂停或停止，`false` 表示正在播放。通过该状态判断按钮的点击事件是触发播放还是暂停。
...
这两个 API 有部分功能不一致，需要做额外的处理。比如， `uni.getBackgroundAudioManager()` API 没有 `onSeeked` 事件回调等。
...
`navigator` 对象中的属性和方法在不同的浏览器中可能会有所差异，所以在使用之前最好检查其兼容性或使用相关的 polyfill 库来保证代码的正确运行。
:::
...
部分旧版浏览器可能不支持 Fetch API，使用 XMLHttpRequest 来替代。
:::
