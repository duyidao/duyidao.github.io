---
layout: doc
title: 华润水泥 图片可拖拽对比钩子
titleTemplate: 华润水泥 图片可拖拽对比钩子
description: 华润水泥 jeecg 后台管理 低代码 图片可拖拽
head:
  - - meta
    - name: description
      content: 华润水泥 图片可拖拽对比钩子
  - - meta
    - name: keywords
      content: 华润水泥 jeecg 后台管理 低代码 图片可拖拽
pageClass: lingsi-jeecg-comparison
tags: releaseCapture,setCapture,wheel
---
...
<template>
  <img
    class="bigger_img_left"
    :src="$SN_DESIGN_BASE_IMG + imgs[0].imgUrl"
    ref="leftImg"
    :style="leftStyle"
    :width="width + 'px'"
    :height="height + 'px'"
...
- `$nextTick`
  ...
  <word text="Vue" />是异步执行<word text="DOM" />更新的，一旦观察到数据变化，<word text="Vue" />就会开启一个队列，然后把在同一个事件循环 `(event loop)` 当中观察到数据变化的 `watcher` 推送进这个队列。如果这个 `watcher` 被触发多次，只会被推送到队列一次。这种缓冲行为可以有效的去掉重复数据造成的不必要的计算和<word text="DOM" />操作。而在下一个事件循环时，<word text="Vue" />会清空队列，并进行必要的<word text="DOM" />更新。
  当设置 `vm.someData = 'new value'`，<word text="DOM" />并不会马上更新，而是在异步队列被清除，也就是下一个事件循环开始时执行更新时才会进行必要的<word text="DOM" />更新。如果此时你想要根据更新的<word text="DOM" />状态去做某些事情，就会出现问题。。为了在数据变化之后等待<word text="Vue" />完成更新<word text="DOM" />，可以在数据变化之后立即使用 `Vue.nextTick(callback)` 。这样回调函数在<word text="DOM" />更新完成后就会调用。
...
1. 判断当前触发事件的鼠标 `x` 轴坐标，如果小于 50%，则左边图片缩放，反之右边图片缩放（因为两张图片平分所有空间）
2. 判断当前图片的宽高，如果小于等于 0 则停止缩小
      ...
      <div class="bottom">
        <div ref="left_box">
          ...
          <template v-else>
            ...
            <div class="thumbnail">
              <img
                :src="$SN_DESIGN_BASE_IMG + imgs[0].thumbnail"
                width="100px"
                height="100px"
              />
              <div>裁剪效果</div>
        ...
        <div>
          ...
          <template v-else>
            <img
              class="bigger_img_right"
              :src="$SN_DESIGN_BASE_IMG + imgs[1].imgUrl"
              alt=""
              :width="w + 'px'"
              :height="h + 'px'"
              :style="rightStyle"
            ...
            <div class="thumbnail">
              <img
                :src="$SN_DESIGN_BASE_IMG + imgs[1].thumbnail"
                width="100px"
                height="100px"
              />
              <div>裁剪效果</div>
  ...
  methods: {
    ...
    handleDownLoadFn(type, which) {
      ...
      image.onload = function () {
        ...
        a.download = which
          ? that.imgs[type].name + '裁剪图.png'
          : that.imgs[type].name + '.png' // 设置图片名称
