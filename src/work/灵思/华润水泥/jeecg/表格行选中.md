...
<word text="Ant Design Vue" /> 组件库中有一个表格组件 `a-table` ，可实现单选表格或多选表格的效果，但是只有点击单选框或多选框才有效。用户想要点击当前行也能选中该行的数据。

## 思路

查看官方文档有没有实现该需求的 `api` 。官网文档指路：[Table 表格](https://1x.antdv.com/components/table-cn/#API) 。
