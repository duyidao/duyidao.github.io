...
小项目或对代码体积有要求的项目，可能不希望引入<word text="Pinia" />、<word text="Vuex" /> 等全局状态管理库，而是自己实现一个简单的全局状态管理。

## 实现

通过<word text="闭包" />将状态存储，使用时拿到之前存储的值。第一次调用时无数据，触发一次回调函数；后续调用时直接返回之前存储的返回值。
