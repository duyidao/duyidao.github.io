...
封装方法可以用到<word text="Vue3" />的 `hook` 思想，新建一个 `.js` 文件，创建一个函数，在其中创建变量和方法，最后 `return` 返回出去供外部使用。基础代码如下：
...
<word text="Storage" />本身是不支持过期时间设置的，要支持设置过期时间，可以效仿<word text="Cookie" />的做法，在存储的时候保存当前的时间戳，在获取的时候获取时间戳来实现时效性。
...
信息加密有很多种，比如基础的 `base64` 加密、`md5` 加密等。这里引用 `crypto-js` 第三方库作为加密方式，使用方式为设置密钥，密钥通过 `md5` 加密生成。
