...
现有一个项目需求需要使用提示弹窗组件，多个页面组件需要使用。

该弹窗组件在封装时需注意：

1. 弹窗显示时机不一致，可能在 A 页面一进入就显示，在 B 页面点击按钮显示
2. 弹窗同意或拒绝按钮执行的后续不一致，拒绝隐藏弹窗，同意调用接口，后续操作看各自组件需求
3. 弹窗组件内部内容不一致

如果直接使用多个组件，页面上就需要写很多个组件，声明很多个控制显隐的变量，代码冗余且不易维护。

### render 方案

把弹窗封装为一个组件，点击行为传给组件，通过 `v-if` 控制显隐。
...
现在页面上就有无样式效果的弹窗了。但这种方法不推荐，更推荐使用 JSX。
...
需注意的是，有些场景可能用不到 `cancel` 之类的按钮点击事件，没有传对应函数方法，需要有良好的代码健壮性意识，添加非空判断，避免代码报错。

### h 与 createApp 方案

这个写法与前面的 JSX 方案类似，只不过不使用标签方式，而是使用 <word text="Vue" /> 提供的 `h` 和 `createApp` 方式。

首先声明一个函数，接收三个参数：第一个参数是弹窗内部要渲染的组件（这里以 Element Plus 组件的弹窗为例），第二个参数是内部组件的 `props`，第三个参数是弹窗组件的 `props`。通过 `h` 创建完虚拟 <word text="DOM" /> 后，再通过 `createApp` 创建一个 <word text="Vue" /> 实例，最后通过 `mount` 方法挂载到 `body` 上。
...
执行后发现页面只打开了 `dialog` 弹窗，没有 `form` 表单组件。查看控制台发现有警告说组件找不到。这是因为 `createApp` 创建的实例需要 `use` 方法来注册组件和方法（如组件库组件、`pinia`、`router`），之前 `template` 的写法用的是 `main.ts` 入口文件的 `createApp` 实例，组件和方法都已注册；但这里是新建了一个 `createApp` 实例，没有注册其他组件库的组件，所以报警告。

解决方法是，在 `signProp` 函数中通过 `app.use` 方法来注册组件库的组件。可一个个复制粘贴过来，如果代码量少这样很方便，但代码量多会很繁琐，且不利于后续维护，更好的方法是写成一个函数，在函数内注册挂载。
