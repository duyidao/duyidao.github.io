...
2. 绑定的是什么类型的事件？
   ...
   事件有很多，`click`、`mousemove`、`keydown` 等，需使用者传递。
...
先写一个简易版函数，接收四个参数：`el`、`type`、`callback`、`option`，分别表示元素、事件类型、回调函数、是否冒泡等。
...
效果上没问题，但在使用上开发者缺乏良好的 <word text="TypeScript" /> 提示，需要为函数添加类型。函数重载是一个很好的方案。
...
关于 <word text="Vue" /> 的 `watch` 方法，第二个参数是一个回调函数，接收三个参数：
