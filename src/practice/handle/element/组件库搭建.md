...
前端开发时，常常会搭建一个自己的组件库。搭建一个组件库最基础的步骤总览：
...
新建一个组件库文件夹，推荐先新建一个 `package` 文件夹，再在里面新建，原因是后续可能会用到其他工具。本次新建了一个 `MyUI` 文件夹。
...
> [!WARNING] 注意：
>
> 1. 定义好 `Workspace` 后，需要 `npm i` 一下，无论之前是否已 `npm i`
...
接下来思考如何实现全局引入。全局引入一个组件时，都是通过 `use()` 注册组件。
...
`use()` 本质是传一个对象进去，它会调用 `install` 方法，执行方法时会传入一个参数 <word text="Vue" />，可通过该参数执行 `mixin` 混入和 `component` 注册组件等。
...
最后是打包组件库 `myui` 的代码，因此需要在 `package.json` 文件中添加一个新的打包命令。
