...
这么做虽可穿透事件与方法，但父组件没有代码提示，只能手敲或翻阅文档，不利于开发，也失去了 <word text="TypeScript" /> 的意义。因此需要转换思路。

Element Plus 组件库导出提供了相关组件类型，可借助此类型获取代码提示。
...
现在父组件使用时能看到相应提示，但出现了 <word text="TypeScript" /> 报错，提示参数是必传的，需要使用 <word text="TypeScript" /> 的 `Partial` 类型包裹。

> `Partial` 作用是将类型中的所有属性变为可选。

目前只考虑了属性，还没考虑到事件。事件都在 `$attrs` 中，因此可用扩展运算符浅拷贝的方式把 `$attrs` 和 `props` 合并。
...
`useAttrs` 和 `useSlots` 是 <word text="Vue3" /> 提供的钩子函数，可获取 `$attrs` 和 `$slots`。
...
可转变思路，不直接使用 `<el-input></el-input>` 挂载组件，而是通过 `<component :is=""></component>` 挂载组件。`is` 不仅可给一个子组件，还可给一个 `h` 函数，因此可借助 `h` 函数来挂载插槽。
...
组件方法暴露可使用 `ref` 暴露方法，通过 `Object.assign` 合并对象的方式暴露出去。
...
但这种方法不可取，如果组件绑定了 `v-if`，后续存在变动的情况，就拿不到 `inputRef` 的值了。

### Proxy 代理暴露

既然 `defineExpose` 要传一个对象，那可不可以用 `new Proxy` 来代理？外部使用子组件时触发代理的 `get` 方法，把子组件实例返回出去。
...
但这样还是不够，在父组件仍然拿不到子组件暴露出去的方法。这是因为 <word text="Vue" /> 在底层封装获取代理对象时，还会判断 `proxy.has()`，如果 `proxy.has()` 返回 `false`，则不会获取到代理对象。
