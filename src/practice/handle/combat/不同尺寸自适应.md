...
1. <word text="VSCode"/> 下载插件 `px to rem` ，可以自动将 `px` 转换为 `rem`
2. 使用 <word text="PostCSS"/> 插件 `postcss-pxtorem` ，自动将 `px` 转换为 `rem`。如果不想要某些 `px` 转换为 `rem` ，可以采用 `style` 行内式
...
可以使用对应的库 <word text="PostCSS"/> 插件 `postcss-px-to-viewport` 自动转换。
...
`rem` 不去监听 `resize` 事件，去修改 `fontSize` 是做不到在动态改变屏幕宽度时变化大小的。如果不想监听 `resize` 事件，或变化触发不了 `resize` ，用 `vw`。

### 百分比

百分比并非无用，`rem` 和 `vw` 都是根据屏幕去算的，如果需要根据父组件缩放就必须用百分比了。
