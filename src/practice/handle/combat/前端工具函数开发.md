...
该版本存在以下缺陷：最多只能转换到十万，且无法添加单位，代码可读性和可维护性都很差。
...
但仍有问题：后续新增单位仍需人为维护，如果遇到冷门单位，只有部分组件需要，直接添加在 `Map` 里维护成本较高。
...
扩展性已具备，但方便性不足。一个页面内单位通常是统一的，但工具函数每次调用都需传单位，没有记忆功能；另外，某些项目有自己的常用单位，默认映射中没有，每次都要手动传。
...
工具函数已具备扩展性与方便性，但健壮性仍有不足：

1. 如果使用者传的值不是数字，则工具函数会报错。
2. 如果使用者传的单位是映射内没有的，则工具函数会报错。
3. 其他情况......

基础做法：函数需要添加类型校验判断，如果 `value` 通过 `Number` 转换后得到的是 `NaN` ，则抛出异常；如果单位没有在映射表中，也抛出异常报错。
