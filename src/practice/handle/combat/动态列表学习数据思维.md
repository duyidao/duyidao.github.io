...
可以同时勾选、取消勾选展示对应列的表格，有一部分列允许显示隐藏，一部分列不允许显示隐藏。同时能本地存储上次的配置、表格内容支持通过<word text="slot" />自定义的需求。
...
通过<word text="v-for" />循环数组，渲染每一列。但这么做有一个缺点，如果不需要某个配置，则需要直接修改数组对象，后续需要时找不回来，会随着操作丢失一部分。因此该方案不适用。
...
根据上方思路的方案二，通过<word text="v-for" />循环 `state` 对象，<word text="v-if" />判断 `state` 对象的值来决定是否显示该列。这样即使不显示该列，该列的配置信息也不会丢失，可以随时恢复显示。
...
这个方法前期可行，但后续维护中如果需要多种效果，则会有很多 `v-if` 判断，代码会显得很冗余。
...
相比第一个方案，虽然需要新增很多子组件维护，但是父组件不再有过多的 `v-if` 判断，减少代码的冗余量。

第三种方法是通过<word text="render" />或<word text="JSX" />的方法来实现，这里采取<word text="render" />的方法。首先与方法二一样，需要创建一个子组件，接收父组件的参数和<word text="render" />函数，返回的是<word text="render" />方法和<word text="h" />方法创建的 DOM。父组件的 `columnConfig` 对象新增 `render` 属性，值是一个函数，传递三个参数，参数一是该 DOM 的标签，参数二是一个对象，该 DOM 的元素（如类名、id 等），参数三是标签的文本。父组件的 `el-table-column` 组件的 `#default` 插槽引入子组件后，传递 `row` 和对应属性的 `render` 参数。
...
> [!TIP] 题外话
> 这里的 `render` 返回的是一个方法，查看官方文档，`h` 方法的代码是 `return h('div', 123)` ，实际上相当于以下代码的缩写：
>
> ```js
> return () => {
...
function setStateStroe(state) {
  // [!code focus]
  <word text="LocalStorage" />.setItem("table_state", JSON.stringify(state)); // [!code focus]
} // [!code focus]

const state = <word text="reactive" />({}); // 是否显示该列
if (<word text="LocalStorage" />.getItem("table_state")) {
  // [!code focus]
  state = <word text="reactive" />(JSON.parse(<word text="LocalStorage" />.getItem("table_state"))); // [!code focus]
} // [!code focus]
else {
  // [!code focus]
  Object.keys(columnsConfig).forEach((key) => {
