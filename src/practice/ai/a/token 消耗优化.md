...
`token` 的消耗可理解为：每次提问传给智能体接口的字符越多，回答越长，消耗的 `token` 越多。

每次提问消耗的 `token` = **传给大模型接口的 message 总字符量 + 大模型接口回答的字符量**。虽然 `token` 并非按字符串量精确计算，但可如此近似理解。给大模型接口的字符串越多，`token` 消耗越大。

为保障上下文，每轮问答都会存入 message 并推给智能体接口。用户持续提问将导致 `token` 消耗急剧增长。

## token 优化

核心在于限制对话记录长度，移除久远且无需再消耗 `token` 的历史对话。
