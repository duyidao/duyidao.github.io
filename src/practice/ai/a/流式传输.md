...
<word text="SSE" />技术是一种基于 `http` 的传输技术，他可以让接口不断地给前端发送消息。而不是传统 `http` 的一定要一次性返回，返回就断开。在<word text="SSE" />方案下，接口还是 `http` 接口，但是可以把要返回的内容的拆开，一部分一部分的下发。
...
但是这种方式很不合适，主要是以下几个方面：

1. 只能是 `get` 请求
2. `token` 必须带着请求头上，很不安全
3. 前端处理很麻烦

## 成熟方案

1. 接口侧把调用大模型的地方加上 `stream` 为 `true`，并按读取流的方式读取内容
2. 前端直接用库 `@microsoft/fetch-event-source`

这个库可以解决上述问题，并且使用起来也很简单。
