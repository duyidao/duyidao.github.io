...
这个效果图有多种实现方法，但最快的实现方式，还是通过 `box-shadow` 内阴影来实现。
...
想要绘制一条曲线，可能的方法是借助 `clip-path`；或者使用 `text-decoration` 的波浪下划线 `wavy`；更甚至，使用渐变叠加来实现。
...
常规思路是新增一个遮罩层 `div` ，覆盖背部，然后弹窗组件通过定位布局，放到对应的位置；又或者使用伪元素 `::before` 等。
