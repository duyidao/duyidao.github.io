...
再看 [用 mask 实现融合效果](/read/CSS%20技术揭秘与实战通关/巧用遮罩解决各类视觉难题#技巧三-用-mask-实现融合效果) 的例子，两个图片斜角融合，也可以通过修改 `mask: linear-gradient` 的参数实现类似的动画堆叠效果。
...
对于 `mask` 的使用，一直都是用 `background` 的渐变属性作为属性值，但其实 `mask` 还能用 `svg` 、 图片等作为遮罩的内容。
...
这个转场动画看似复杂，但核心是下面这张雪碧图：

![核心雪碧图](https://pic1.imgdb.cn/item/68f087fec5157e1a887835aa.png)

实现原理：利用透明叠加部分不可见的原理，逐帧改变其不可见的部分，最终完全展示出图片。
