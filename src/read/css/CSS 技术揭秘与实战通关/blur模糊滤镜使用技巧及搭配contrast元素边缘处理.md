...
在关于 `filter` 和 `backdrop-filter` 的区别对比中，讲到了[作用对象的差异](/read/CSS%20技术揭秘与实战通关/filter的三种滤镜模式#作用对象的差异)，其中就有毛玻璃效果，也讨论了 `filter: blur()` 和 `backdrop-filter: blur()` 的差别，也查看了通过 `backdrop-filter: blur()` 实现的毛玻璃效果。

`backdrop-filter` 滤镜很多浏览器一度不支持，因此很少使用。现在多数浏览器已支持该语法，这也是它能大规模使用的原因。

#### 利用模糊实现彩色阴影 / 渐变阴影

在阴影的章节中，讨论了如何使用模糊效果实现[彩色阴影和渐变阴影](/read/CSS%20技术揭秘与实战通关/css%20阴影#彩色阴影-渐变阴影)，以及高级技巧[利用阴影复制自身图形](/read/CSS%20技术揭秘与实战通关/shadow的高级技巧#技巧一-利用阴影复制自身图形)。

回顾一下知识点，如何给下面这张图片添加一个彩色阴影。

![要添加彩色阴影的图片](https://pic1.imgdb.cn/item/691d3bab3203f7be0015b663.png)

核心思路：为这个图片元素的伪类 `::after` 添加 `background: inherit;` ，创建一个同样图片内容的副本；然后设置 `filter: blur()` 让它模糊，就能模拟阴影效果实现彩色阴影了。
...
这个主色调的使用场景非常之多，可以利用它们来填充背景等：

![主色调的使用场景](https://pic1.imgdb.cn/item/691d5b803203f7be001677ea.png)

以前要实现类似的需求，需要用 Canvas 进行取色、一系列计算、再平均色值得出最终结果。
...
在组件库或者一些网站中，鼠标 `hover` 悬停在按钮组件或者一些图片上，会有一些颜色的对比度变化。可能有人以为是换了颜色，实际上变化的是 `contrast()` 对比度。
