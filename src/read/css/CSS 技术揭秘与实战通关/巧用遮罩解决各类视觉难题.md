...
在前面的章节 [渐变可以叠加多层](/read/CSS%20技术揭秘与实战通关/background的8种核心技巧#技巧三-渐变可以叠加多层) 中介绍了渐变可以叠加多层，且 `mask` 属性也可以使用 `background` 的渐变属性，那么就可以使用这个技巧实现图片的裁剪。
...
前面章节 [渐变颜色可以设置透明色](/read/CSS%20技术揭秘与实战通关/background的8种核心技巧#技巧一-渐变颜色可以设置透明色) 中，利用伪类设置线性渐变让末尾透明来实现渐隐，现在可以利用 `mask` 属性来实现渐隐效果。
