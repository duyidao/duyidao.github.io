...
上一章讲述模块的语法，本章重心在浏览器和 `Node` 之中如何加载 ES6 模块。
...
CommonJS 模块格式的加载原理：一个模块就是一个脚本，`require` 命令第一次加载该脚本时就会执行整个脚本，然后在内存中生成一个对象。也就是说，CommonJS 模块无论加载多少次，都只会在第一次加载时运行一次，以后再加载时就返回第一次运行的结果，除非手动清除系统缓存。
