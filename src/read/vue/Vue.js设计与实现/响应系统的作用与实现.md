...
除了前面提到的编译器和渲染器，响应系统也是 <word text="Vue.js"/> 的核心组成部分。响应系统的主要作用是让 <word text="Vue.js"/> 应用程序在数据发生变化时，自动更新视图。想要提响应系统，就要从响应式数据和副作用函数这两个概念开始。
...
<word text="JavaScript"/> 中，读取对象会触发其 `get` 方法，修改对象会触发其 `set` 方法。因此，可以通过重写对象的 `get` 和 `set` 方法来实现响应式数据。读取对象属性时，将副作用函数与属性进行关联；修改对象属性时，执行所关联的全部副作用函数。在 <word text="ES5"/> 之前，只能通过 <word text="Object.defineProperty"/> 来实现，<word text="ES6"/> 之后可以使用 <word text="Proxy"/> 代理。
...
const obj = new Proxy(data, {
  // 读取操作
  get(target, key) {
    bucketSet.add(effect)
    return target[key]
  },
  // 修改操作
  set(target, key, value) {
    target[key] = value
    bucketSet.forEach((fn) => fn())
    return true
  },
})
```
...
const obj = new Proxy(data, {
  // 读取操作
  get(target, key) {
    if (activeEffect) bucketSet.add(activeEffect) // 把全局保存的副作用函数收集保存 // [!code focus]
    return target[key]
  },
  // 修改操作
  set(target, key, value) {
...
可以得出，根据这个树型结构建立联系后，只需要触发对应的副作用函数即可。想要实现这个功能，要用 <word text="WeakMap"/> 替代 <word text="Set"/> ；然后收集对象的全部属性字段 <word text="Map"/> ，每个 <word text="Map"/> 对应一个属性字段；最后用 <word text="Set"/> 收集属性字段对应的副作用函数。

::: code-group

```js [proxy.js]
const bucketWeakMap = new WeakMap() // [!code focus]
...
const track = (target, key) => {
  ...
  let depsMap = bucketWeakMap.get(target) // 获取WeakMap映射对应的target对象 // [!code focus]
  if (!depsMap) bucketWeakMap.set(target, (depsMap = new Map())) // 如果没有，则为该target对象新建一个Map映射用于保存所有属性字段key // [!code focus]
...
const trigger = (target, key) => {
  const depsMap = bucketWeakMap.get(target)
  if (!depsMap) return
...
<body>
  ...
  <script>
    ...
    const bucketWeakMap = new WeakMap() // 存储依赖函数

    // track函数收集依赖
    const track = (target, key) => {
      if (!activeEffect) return
      let depsMap = bucketWeakMap.get(target)
      if (!depsMap) bucketWeakMap.set(target, (depsMap = new Map()))
...
const track = (target, key) => {
  if (!activeEffect) return
  let depsMap = bucketWeakMap.get(target) // 获取WeakMap映射对应的target对象
  if (!depsMap) bucketWeakMap.set(target, (depsMap = new Map())) // 如果没有，则为该target对象新建一个Map映射用于保存所有属性字段key
