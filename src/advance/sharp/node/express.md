...
1. `express.static` 快速托管静态资源的内置中间件，例如：<word text="HTML" />文件、图片、<word text="CSS" />样式等（无兼容性）
...
CORS （`Cross-Origin Resource Sharing`，跨域资源共享）由一系列 HTTP 响应头组成，这些 HTTP 响应头决定浏览器是否阻止前端<word text="JavaScript" />代码跨域获取资源。
...
在编写调试<word text="Node" />项目的时候，如果修改了项目的代码，则需要频繁的手动 `close` 掉，然后再重新启动，非常繁琐。
