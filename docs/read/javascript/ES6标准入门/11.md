...
ES6中新增了两种数据结构：Set和Map，以及它们的弱引用版本WeakSet和WeakMap。`Set` 是一个元素唯一、无序且允许重复的数据结构，通过 `new Set()` 创建，支持 `add` 、`delete`、`has` 等方法，并可通过 `for...of` 循环遍历。`Map` 是键值对的集合，键不仅限于字符串，支持 `set`、`get`、`has`、`delete` 等方法，同样可通过 `for...of` 循环遍历。`WeakSet` 和 `WeakMap` 分别对应 `Set` 和 `Map` 的弱引用版本，只接受对象作为键，且键的引用为弱引用，有利于垃圾回收，但不可遍历且没有 `size` 属性。
