...
![一图流](https://s1.ax1x.com/2023/07/21/pCbSoU1.png)
...
const onSubmit = async () => {
  if (!fileObj.value) fileObj.value = JSON.parse(localStorage.getItem(fileObj.value.name)).file
  ...
  while(current < fileSize) {
    ...
    if (res.code === 200) {
      ...
      // 存储当前文件信息
      localStorage.setItem(fileObj.value.name, JSON.stringify({ file: fileObj.value, current }))
    }
  }
}
</script>
