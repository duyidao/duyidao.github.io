...
首先认识一下无头浏览器，无头浏览器是一个没有 UI 界面的模拟浏览器，可以用 <SPW text="NodeJS" /> 配合对应的库模拟用户在浏览器浏览的操作。

常见的无头浏览器有：<SPW text="Puppeteer【推荐】" />、<SPW text="PhantomJS【老牌，不维护】" />、<SPW text="Selenium" /> 等。

## 思路实现

1. 编写 <SPW text="NodeJS" /> 脚本代码
2. 脚本 `link` 为全局指令（可不做）
3. `js` 代码立体无头浏览器模拟访问
4. 获取各项指标
...
想要获取项目各项指标，无需手动记录时间去计算，`performance` 对象可以帮我们记录各项指标，不过此时是在 <SPW text="Node" /> 环境，<SPW text="Node" /> 环境没有这个事件对象，因此需要搭配 `page.evaluate` 方法，将 `performance` 对象传递到浏览器环境，在浏览器环境获取各项指标。
...
> 2. 如果想把页面的对象传到 <SPW text="Node" /> ，则需要通过 `JSON.stringify` 返回回来。

## 可做的事

1. 最常见的页面首屏渲染，<SPW text="DOM" /> 渲染时间等
