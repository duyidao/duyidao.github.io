...
`FormData` 是一个用于构建和处理表单数据的 <SPW text="JavaScript" /> 对象。它可以方便地将表单数据格式化为一个可以通过 <SPW text="XMLHttpRequest" /> 或 <SPW text="Fetch API" /> 发送到服务器的数据体，用于和后端传输的对象。
...
`FileReader` 是一个用于读取文件内容的 <SPW text="JavaScript" /> 对象。它提供了一些方法，可以异步读取文件数据并以不同的方式进行处理，例如文本、二进制数据、 `base64` 或数据 `URL`。
