...
2. 文本部分不同的样式文字通过函数返回
3. 利用 <SPW text="JSX" /> 根据不同栏目渲染不同的样式
4. 修改源数据
...
该方式简单粗暴，缺陷是 <SPW text="HTML" /> 代码量多且冗余，后续不易维护，因此不是很推荐使用。
...
虽然 `template` 内的代码量减少了，但是函数被执行了多次，且在操作 `num` 的点击事件时，因为该 <SPW text="Vue" /> 文件的渲染依赖于变量 `num` ，在 `num` 发生变化时，整个组件都会触发更新，因此函数方法都会重新执行，造成一定的性能压力。
