...
1. 可以安装对应的 <SPW text="Vue" /> / <SPW text="React" /> 组件，直接作为组件使用。或者直接下载做原生操作
2. 报找不到文件的错误，需要把 `tinymce` 复制到 `public` 文件夹内

### 安装

- 安装 <SPW text="Vue" /> 模块的组件

  ```sh
  yarn add @tinymce/tinymce-vue
  ```

  如果是 <SPW text="React" /> 项目则改为 `@tinymce/tinymce-react` 。
...
- 复制文件到 `public` 文件
  ...
  此时它加载完后会试图加载它的 <SPW text="JS" /> 文件和 <SPW text="CSS" /> 文件，因此要把 `node_modules` 内它的整个内容复制到 `public` 文件内。否则会报错。
...
下载中文包，解压 `langs` 文件夹到 `public` 文件夹内，在配置项中通过 `language` 属性赋值刚解压的 <SPW text="JavaScript" /> 文件名。
...
通过 `tinymce.activeEditor.getContent()` 获取内容。这么获取的内容是 <SPW text="HTML" /> 富文本内容，如果想要获取纯文本内容，则需要在括号内设置 `{format: 'text'}` 。
