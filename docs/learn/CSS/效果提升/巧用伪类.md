...
**`:placeholder-shown`** [CSS](https://developer.mozilla.org/zh-CN/docs/Web/CSS) [伪类](https://developer.mozilla.org/zh-CN/docs/Web/CSS/Pseudo-classes)表示当前显示[占位符文本](https://developer.mozilla.org/zh-CN/docs/Web/HTML/Element/input#placeholder)的任何 `input` 或 `textarea` 元素。用大白话来说就是当输入框标签设置了 `placeholder` 占位符且没输入内容（显示占位符内容）时，该伪类的样式生效。
